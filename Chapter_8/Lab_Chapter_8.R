...
# We see that lstatand rm are by far the most importnat variables. We can also produce partial
#   dependence plots for these two variabels.These pltos illustrate the amrginal effect of
#   the selected variabels on the response after integrating out the other variables.
#
# In this case we can say that median house prices are increasing with rm and decreasing with
#   lstat.
...
# RESOURCES FOR LEARNING GRADIENT BOOSTING METHOD
# https://medium.com/mlreview/gradient-boosting-from-scratch-1e317ae4587d
# https://www.kaggle.com/grroverpr/gradient-boosting-simplified/
# Antoher best resource
# http://blog.kaggle.com/2017/01/23/a-kaggle-master-explains-gradient-boosting/

######### BEST STEP BY STEP EXPLAINGING GBM according to Model in WIKIPEDIA
# https://datascience.stackexchange.com/questions/9134/gradient-boosting-algorithm-example?rq=1



# Basically what happens in gbm is that first we fit a weak model to predict and then using
#   that weak model we then calculate the errors. After calculating the errors we then
#   try to fit the model on errors itself, And we keep on iterating and fitting more models
#   on these errors

# ALSO REMEMBER THAT gradient boosting will add the trees sequentially while bagging and
#   random forests add trees in parallel manner.
...
# These plots provide us how the response varies as our predcitos lstat and rm varies


# Prediction
# We will now use this model to predcit on the new dataset, which is our test dataset

# Here in the predictions we have to give another argument besides the model and newdata
#   that is n.trees which will let us use until which iteration of tree we want to use
#   for prediction. We can only use until how many trees we want by giving this argument
#
boost_preds = predict(boost_boston, newdata = bostondata[-train,], n.trees = 5000)

# Mean squared error is 10.81 which is very low from what we got earlier
#   In random forest we got error as 11.4% and here it has decreased by almost one percent
#   which is very huge.
mean((boost_preds - bostondata$medv[-train])^2)



####### Using Different SHRINKAGE PARAMETER lambda
####### THe default value is 0.01, which says how much the earlier result to influence the new
# ones. We can change it by specifying another argument called shrinkage

boost_boston_shrinkaged = gbm(medv~.,
...
# Verbose means a logical indicating whether or not to print out the progress and performance
#   indicators. If this option is left unspecified , it uses verbose from object. And its
#   default value is false

boost_boston_shrinkaged

boost_preds_shrinkaged = predict(boost_boston_shrinkaged,
                                 newdata = bostondata[-train,],
                                 n.trees = 5000)

# THe mean squared error here is 10.89%
# MSE IS INCREASED when the lambda value is increased from 0.01 to 0.2
mean((boost_preds_shrinkaged - bostondata$medv[-train])^2)
