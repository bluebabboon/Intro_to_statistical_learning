...
# sometimes you want to create a list of numbers for iterating over or couting,following function and notation will be helpful
x=seq(1,9)                
x                         # both seq and the method below gives the same result
x=1:9
x
# we can also use seq to specify the stepping in between two integers
x=seq(-5,5,length=20)     # here we want 20 equally spaced elements between -5 and 5 and the result vector will be stored in x
x

########### 3D maps an plots #################

# we will create more sophisticated plots.The contour() function produces contour plot to represent 3dimensional data
...
# outer works like this (x,y,f(x,y)) takes x and y as arugument and gives a array with dimension c(dim(x),dim(y)) where each element of array is function of 
...
# Takes two arguments theta and phi apart from x,y,z;Those are angles which control how to view the plot
...
A[c(1,3),c(2,4)]         # this command is selecting 1,3 rows and 2,4 columns and returns a 2x2 matrix that are intersection of both selected rows and columns

A[1:3,2:4]               # this command selects 1:3--> 1,2,3 rows and 2:4--> 2,3,4 columsn and gives the matrix that is intersection of them

A[1:2,]                  # We can leave a blank after , to select all the columns but only specified rows

A[,1:2]                  # this command selects all rows but only 1,2 columns,same as above but in reverse

A[-2,-3]                 # this command will say that dont select that particular rows and columns and return everything else in a matrix or whatever

A[c(-1,-3),c(-2,-4)]     # specifying mutliple columns and rows

A[-c(1,3),-c(2,4)]       # same command as above ,just different notation, "-" can be kept outside array too
...
?fix                       # fix(data_object) will open a data editor in new window to edit the data in GUI 

fix(Auto_data)

# Notice that calling read.table() loaded variable names as part of data in first row
# To avoid that we give another option saying that it has header and add another argument "header=TRUE" in read.table() function

Auto_data = read.table("data/Auto.data",header = TRUE)  # adds header and will not consider first row as part of data object
...
# To say that data is missing and convey this to R we add another argument " na.strings="?" " in read.table()
...
dim(Auto_data_csv)            # Notice that we have 5 rows with missing values and those are deleted from the data object

typeof(Auto_data_csv)         # read.csv() will generate a list data object ,it is different from matrix

names(Auto_data_csv)          # Whatever the headers we specified ,the feature names can be accessed by names(data_object) command


#################### Additional Graphical and Numerical Summaries ############

plot(cylinders,mpg)            # Auto_data_csv object has columns with names cylinders and mpg,but we cannot directly access them

# unless we specify them in the following way

plot(Auto_data_csv$cylinders,Auto_data_csv$mpg)        # adding "$" symbol wil let us access individual columns from the list object

Auto_data_csv[c("cylinders","mpg")]                    # another way of accessing individual columns in a list object,   data_object[c(column1_name,column2_name,..)]

# if we are tired of specifying from the data object by explicitly specifying column names inside data object we can use attach()
...
plot(cylinders,mpg)             # Here inside the plot the names of x and y axes are set to feature names

# we can also change the feature types like,from integer to categorical or double,by using the following genre of function
...
plot(cylinders,mpg)             # Notice how the plot has been changed,previously it is scatter plot,now it is box plot
...
plot(cylinders,mpg,col="red",     # Rotates the plot to view the boxes horizontally,previously they are vertical,Also "T" can be used instead of "TRUE" anywhere
...
hist(mpg)                                   # Histogramns are just graphical representation of each values occurence in a bar type plots,total sum of all occurences = total number of rows in that feature

hist(mpg,col = "red",breaks = 15)           # we can add extra arguments as well,col="some color name",breaks=15
                                            # breaks says that lets take max and min value and break it into 15 item sets,equally spaced and calculate frequency for each set
...
hist(as.integer(cylinders))                 # we can use as.integer() to convert cylinders to integer and then plot
                                            # Remember as.integer(feature) will not change feature itself ,it has to be assigned to a new one

hist(cylinders)                             # See the cylinders type itself is not changed ,but as.integer() returns a feature array which are integers
...
pairs(~mpg+displacement+horsepower          # If we want to have scatter plot between selected few features,we can use this way
      +weight+acceleration)               

# Identify() function is a useful interactive method for identifying value for a paritcular variable for points on plot.

plot(horsepower,mpg)

identify(horsepower,mpg,name)               # After printing the plot we call identify with 3 arguments,x axis,yaxis and the value we like to see in interactive mode
                                            # has to press "esc" after finishing picking the points form the plot to exit the interactive mode (This is on Windows)
...
summary(Auto_data_csv)                      # gives data such as Min,Max,Mean,Median,1st Qu,3rd Qu
                                            # 1st quartile -- Means 25% of observations are below this quantity(approx)
                                            # 3rd quartile means 75% of observations are below this quantity(approx)
                                            # for qualitative variables such as name it will list no. of observations that will fall in each category 
